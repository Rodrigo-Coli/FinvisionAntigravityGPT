...
import { Check, X, Save, Loader2, Sparkles, Settings, Users, Tag, Shield, ChevronDown, ChevronRight, Plus, Trash2, Copy } from 'lucide-react';

// Master list of ALL features — add new features here and they auto-appear in every plan
...
export default function AdminPlans() {
  ...
  const updatePlanField = (planId: string, field: string, value: any) => {
    setPlans(prev => prev.map(p =>
      p.id === planId ? { ...p, [field]: value, _dirty: true } : p
    ));
  };
  ...
  const toggleFeature = (planId: string, featureKey: string) => {
    setPlans(prev => prev.map(p => {
      ...
      const features = { ...p.features, [featureKey]: !p.features[featureKey] };
      return { ...p, features, _dirty: true };
    }));
  };
  ...
  const saveCoupon = async () => {
    ...
    let plan_override_id = null;
    if (newCoupon.plan_override_slug) {
  ...
  return (
    <div className="max-w-[1400px] mx-auto px-4 sm:px-10 py-8 space-y-8">
      ...
      <div className="flex gap-1 p-1.5 bg-slate-50 border border-slate-100 rounded-2xl w-fit">
        {[
          { id: 'plans', label: 'Planos', icon: <Settings size={14} /> },
          { id: 'coupons', label: 'Cupons', icon: <Tag size={14} /> },
          { id: 'users', label: 'Usuários', icon: <Users size={14} /> },
        ].map(tab => (
          <button key={tab.id} onClick={() => setActiveTab(tab.id as any)}
            className={`flex items-center gap-2 px-5 py-2.5 rounded-xl text-xs font-bold transition-all ${
              activeTab === tab.id ? 'bg-white text-brand-600 shadow-sm' : 'text-slate-400 hover:text-slate-600'
            }`}>
            {tab.icon} {tab.label}
          </button>
        ))}
      ...
      {activeTab === 'plans' && (
        <div className="space-y-4">
          {plans.map(plan => {
            ...
            return (
              <div key={plan.id} className={`bg-white rounded-[28px] border shadow-sm overflow-hidden transition-all ${
                plan._dirty ? 'border-brand-200' : 'border-slate-100'
              }`}>
                {/* Plan header */}
                <div className="p-6 flex items-center gap-6">
                  <button onClick={() => setExpandedPlan(isExpanded ? null : plan.id)}
                    className="flex items-center gap-4 flex-1 text-left">
                    <div className="w-10 h-10 bg-slate-900 rounded-xl flex items-center justify-center text-white text-xs font-bold uppercase">
                      {plan.slug[0]}
                    </div>
                    <div>
                      <h3 className="font-bold text-slate-900">{plan.name}</h3>
                      <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                        {formatPrice(plan.price_cents)} · {plan.ai_scans_limit === -1 ? 'IA Ilimitada' : plan.ai_scans_limit === 0 ? 'Sem IA' : `${plan.ai_scans_limit} scans/mês`}
                      </p>
                    </div>
                  </button>

                  {/* Inline edits: price + IA limit */}
                  <div className="flex items-center gap-3">
                    <div className="space-y-0.5">
                      <label className="text-[9px] font-bold text-slate-300 uppercase tracking-widest ml-1">Preço (centavos)</label>
                      <input type="number" value={plan.price_cents}
                        onChange={e => updatePlanField(plan.id, 'price_cents', parseInt(e.target.value) || 0)}
                    ...
                    <div className="space-y-0.5">
                      <label className="text-[9px] font-bold text-slate-300 uppercase tracking-widest ml-1">IA scans (-1=∞)</label>
                      <input type="number" value={plan.ai_scans_limit}
                        onChange={e => updatePlanField(plan.id, 'ai_scans_limit', parseInt(e.target.value))}
                        className="w-24 h-9 bg-slate-50 rounded-xl px-3 text-sm font-bold text-slate-900 border-none focus:ring-2 focus:ring-brand-400"
                      />
                    </div>
                    <div className="space-y-0.5">
                      <label className="text-[9px] font-bold text-slate-300 uppercase tracking-widest ml-1">Trial (dias)</label>
                      <input type="number" value={plan.trial_days}
                        onChange={e => updatePlanField(plan.id, 'trial_days', parseInt(e.target.value) || 0)}
                        className="w-20 h-9 bg-slate-50 rounded-xl px-3 text-sm font-bold text-slate-900 border-none focus:ring-2 focus:ring-brand-400"
                      />
                    </div>
                    <div className="flex items-center gap-2 mt-5">
                      <label className="text-[10px] font-bold text-slate-400">Ativo</label>
                      <button onClick={() => updatePlanField(plan.id, 'is_active', !plan.is_active)}
                ...
                {/* Feature toggles — expandido */}
                {isExpanded && (
                  <div className="border-t border-slate-50 p-6 space-y-6">
                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                      Funcionalidades do Plano — qualquer nova feature adicionada ao catálogo aparece aqui automaticamente
                    </p>
                      ...
                      <div key={group} className="space-y-3">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2">
                          {FEATURE_CATALOG.filter(f => f.group === group).map(feat => {
                            ...
                            return (
                              <button key={feat.key}
                                onClick={() => toggleFeature(plan.id, feat.key)}
                                ...
                                <div className={`w-5 h-5 rounded-lg flex items-center justify-center shrink-0 ${
                                  enabled ? 'bg-emerald-500 text-white' : 'bg-slate-200 text-slate-400'
                                }`}>
          ...
          <div className="bg-white rounded-[28px] border border-slate-100 p-8 space-y-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
              <div className="space-y-1">
                <label className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">Código</label>
                <input type="text" placeholder="PROMO20" value={newCoupon.code}
      ...
      {/* USERS TAB — placeholder for Phase 2 */}
      {activeTab === 'users' && (
