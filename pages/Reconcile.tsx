    ...
    try {
      ...
      const mapped = (data || [])
        ...
        .map((t: any) => ({
          ...
          installment_number: t.metadata?.installment_info?.number,
          installment_total: t.metadata?.installment_info?.total
        }));
  ...
  const handleConfirm = async (item: ImportedTransaction) => {
    ...
    try {
      ...
      if (importSource === 'bank') {
        ...
        // SE FOR PARCELAMENTO: Gerar parcelas futuras se for a primeira vez que vemos
        if (item.installment_number && item.installment_total && item.installment_total > 1) {
          ...
          if (remainingCount > 0) {
            ...
            const futureSeries = TransactionSeriesUtils.generateSeries({
              date: item.date,
              description: item.description.replace(/\d+\/\d+/, '').trim(), // Limpa o "2/5" da descrição base
              amount: Math.abs(item.amount),
              type: 'EXPENSE',
              accountId: card.id, // O utils usa accountId para card_id se for série de cartão
              category: 'Conciliação'
            }, {
              type: 'INSTALLMENT',
              count: item.installment_total,
              startDate: item.date // A série vai começar a contar a partir desta data
            });

            // Pular as parcelas que já passaram ou são a atual
            ...
            for (const fTx of futureTxs) {
              ...
              await supabase.from('card_transactions').insert([{
                ...
                status: 'POSTED',
                is_manual: false,
                installment_group_id: crypto.randomUUID() // Poderíamos vincular ao mesmo ID se tivéssemos um
              }]);
            }
          }
        }
                        ...
                        <div className="min-w-0 truncate">
                          <p className="text-[9px] font-black text-gray-400 uppercase tracking-widest flex items-center gap-2">
                            ...
                            {item.installment_total && (
                              <span className="bg-amber-100 text-amber-700 px-1.5 py-0.5 rounded text-[8px] font-black">PARCELA {item.installment_number}/{item.installment_total}</span>
                            )}
                          </p>
