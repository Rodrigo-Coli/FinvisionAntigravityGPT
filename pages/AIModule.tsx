...
const AIModule: React.FC<{ user: Profile }> = ({ user }) => {
  ...
  return (
    <div className="max-w-7xl mx-auto py-12 sm:py-16 px-6 sm:px-10 space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-8">
        <div className="space-y-4">
          <div className="flex flex-wrap items-center gap-4">
            <h1 className="text-4xl sm:text-5xl font-bold text-slate-900 tracking-tight">AI & <span className="text-brand-600">Labs</span></h1>
            <span className="px-3 py-1 bg-brand-50 text-brand-600 rounded-full text-[10px] font-bold uppercase tracking-widest border border-brand-100 shadow-sm">Inteligência</span>
          </div>
          <p className="text-slate-500 font-medium text-lg">Sua central de economia inteligente e laboratório de preços.</p>
        </div>
      </header>
      ...
      <div className="flex gap-10 border-b border-slate-100 overflow-x-auto scrollbar-hide">
        {[
          ...
          <button
            ...
            className={`pb-4 px-1 text-xs font-bold transition-all border-b-2 flex items-center gap-2.5 whitespace-nowrap uppercase tracking-widest ${activeTab === tab.id ? 'border-slate-900 text-slate-900' : 'border-transparent text-slate-400 hover:text-slate-600'}`}
          >
            {tab.icon} {tab.label}
          </button>
        ))}
        ...
        {activeTab === 'upload' && (
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 xl:gap-14">
            <div className="lg:col-span-8 space-y-10">
              {!receipt ? (
                <div
                  ...
                  className={`bg-white border-2 border-dashed rounded-[48px] p-20 flex flex-col items-center justify-center transition-all min-h-[460px] cursor-pointer hover:bg-slate-50/50 ${isProcessing ? 'border-brand-400' : 'border-slate-100 hover:border-brand-200'}`}
                >
                  ...
                  {isProcessing ? (
                    <div className="text-center space-y-8">
                      <div className="relative">
                        <div className="w-20 h-20 border-4 border-slate-100 border-t-brand-600 rounded-full animate-spin mx-auto" />
                        <Sparkles className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-brand-600 animate-pulse" size={24} />
                      </div>
                      <div className="space-y-2">
                        <h3 className="text-2xl font-bold text-slate-900 tracking-tight">Processando Cupom...</h3>
                        <p className="text-slate-500 font-medium">A IA está analisando seus itens com precisão.</p>
                      </div>
                    </div>
                  ) : (
                    <div className="text-center space-y-10">
                      <div className="w-24 h-24 bg-slate-50 rounded-[32px] flex items-center justify-center text-slate-200 mx-auto">
                        <Receipt size={48} />
                      </div>
                      <div className="space-y-4">
                        <h3 className="text-3xl font-bold text-slate-900 tracking-tight">O que você comprou?</h3>
                        <p className="text-slate-500 font-medium max-w-sm mx-auto text-lg leading-relaxed">Envie fotos dos seus cupons fiscais para extrair os itens e alimentar sua inteligência de preços.</p>
                      </div>
                      <button className="px-10 py-5 bg-slate-900 text-white rounded-2xl font-bold text-xs uppercase tracking-widest hover:bg-slate-800 transition-all shadow-lg shadow-black/5">Selecionar Arquivos</button>
                    </div>
                  )}
                </div>
              ) : (
                  ...
                  <div className="max-h-[600px] overflow-y-auto divide-y divide-slate-50 scrollbar-hide">
                    {receipt.items.map((item: any, idx: number) => (
                      <div
                        ...
                        className={`p-8 sm:p-10 flex items-center justify-between transition-all cursor-pointer group hover:bg-slate-50/30 ${item.selected ? 'bg-white' : 'bg-slate-50/50 opacity-40 grayscale'}`}
                      >
                        <div className="flex items-center gap-6">
                          <div className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-all ${item.selected ? 'bg-slate-900 text-white shadow-lg shadow-black/5' : 'bg-slate-100 text-slate-400'}`}>
                            {item.selected ? <Check size={20} /> : <Tag size={20} />}
                          </div>
                          <div className="space-y-1.5">
                            <h4 className="font-bold text-slate-900 text-lg sm:text-xl leading-tight tracking-tight">{item.description}</h4>
                        ...
                        <div className="text-right space-y-2">
                          ...
                          {item.category_hint && (
                            <span className="inline-block text-[9px] bg-slate-100 text-slate-500 px-3 py-1 rounded-full font-bold uppercase tracking-wider">
                              {item.category_hint}
                            </span>
                          )}
                        </div>
            ...
            <div className="lg:col-span-4 space-y-10">
              {receipt && (
                <div className="bg-white rounded-[40px] border border-slate-100 shadow-soft p-10 sticky top-28 animate-in fade-in zoom-in-95 duration-500 space-y-10">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h3 className="text-xl font-bold text-slate-900 tracking-tight">Finalizar</h3>
                  ...
                  <div className="space-y-8">
                    <div className="grid grid-cols-3 gap-3">
                      {[
                        ...
                        <button
                          ...
                          className={`p-4 rounded-2xl border transition-all flex flex-col items-center gap-2.5 ${reconcileMode === mode.id
                    ...
                    {reconcileMode === 'partial' && (
                      <div className="p-6 bg-slate-50 rounded-3xl border border-slate-100 animate-in slide-in-from-top-2">
                        <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest block mb-3">Valor para Conciliação ({receipt.currency || 'BRL'})</label>
                        <div className="relative">
                          <span className="absolute left-0 top-1/2 -translate-y-1/2 text-xl font-bold text-slate-300 tracking-tight">{receipt.currency || 'R$'}</span>
                          <input
                            type="number"
                            value={partialValue}
                            onChange={(e) => setPartialValue(Number(e.target.value))}
                            className="bg-transparent w-full text-4xl font-bold text-slate-900 pl-12 focus:outline-none tracking-tighter"
                          />
                        </div>
                      </div>
                    )}
                    ...
                    {receipt.currency && receipt.currency !== 'BRL' && (
                      <div className="p-6 bg-amber-50 rounded-3xl border border-amber-100 space-y-6">
                        <div className="flex items-center justify-between">
                          <span className="text-[10px] font-bold text-amber-900 uppercase tracking-widest">Moeda: {receipt.currency}</span>
                        ...
                        {isApplyingTax && (
                          <div className="space-y-4">
                            <div className="space-y-2">
                              <label className="block text-[10px] font-bold text-amber-700 uppercase tracking-widest">Cotação do Dólar/Euro</label>
                              <div className="relative">
                                <span className="absolute left-4 top-1/2 -translate-y-1/2 text-sm font-bold text-amber-500">R$</span>
                                <input
                                  type="number"
                                  step="0.01"
                                  value={exchangeQuote}
                                  onChange={(e) => setExchangeQuote(parseFloat(e.target.value))}
                                  className="w-full bg-white border border-amber-200 rounded-xl pl-10 pr-4 py-3 text-sm font-bold outline-none focus:ring-2 focus:ring-amber-500 transition-all"
                                />
                              </div>
                            </div>
                            <div className="flex items-start gap-3 p-4 bg-amber-100/50 rounded-2xl">
                              <Info size={16} className="text-amber-700 shrink-0 mt-0.5" />
                              <span className="text-[10px] font-bold text-amber-900 leading-relaxed uppercase tracking-wider">
                    ...
                    <div className="p-8 bg-slate-900 rounded-[32px] text-white shadow-xl shadow-black/5">
                      <div className="flex justify-between items-center mb-2 opacity-60">
                        <span className="text-[10px] font-bold uppercase tracking-widest">Valor Final em BRL</span>
                        <Calculator size={16} />
                      </div>
                    ...
                    <div className="space-y-2.5">
                      <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest block ml-1">Conta de Origem</label>
                      <div className="relative group">
                        <select
                          value={targetAccount}
                          onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setTargetAccount(e.target.value)}
                          className="w-full h-16 bg-slate-50 border border-slate-100 rounded-[20px] px-6 font-bold text-slate-900 text-sm focus:ring-2 focus:ring-brand-500 appearance-none transition-all group-hover:border-slate-200"
                        >
                    ...
                    <button
                      ...
                      className={`w-full h-20 rounded-[28px] font-bold text-xs uppercase tracking-[0.2em] transition-all active:scale-95 flex items-center justify-center gap-4 ${saveStatus === 'done'
                        ? 'bg-emerald-500 text-white shadow-lg shadow-emerald-500/20'
                        : 'bg-slate-900 text-white hover:bg-slate-800 shadow-lg shadow-black/5'}`}
                    >
                      {saveStatus === 'saving' ? <Loader2 className="animate-spin" /> : saveStatus === 'done' ? <Check /> : <Sparkles size={18} />}
                ...
                comparisonData
                  ...
                  .map((product: any) => (
                    <div
                      ...
                      className="bg-white rounded-[32px] border border-slate-100 p-8 shadow-soft hover:shadow-soft-lg hover:border-slate-200 transition-all group relative cursor-pointer active:scale-95 flex flex-col justify-between min-h-[320px]"
                    >
                      <div className="space-y-6">
                        <div className="flex justify-between items-start">
                          ...
                          <span className="text-[9px] font-bold text-slate-400 uppercase tracking-[0.2em] bg-slate-50 px-3 py-1.5 rounded-lg border border-slate-100">{product.category}</span>
                        </div>

                        <h3 className="text-xl font-bold text-slate-900 group-hover:text-brand-600 transition-colors uppercase tracking-tight leading-snug">{product.name}</h3>
                      </div>

                      <div className="space-y-6">
                        <div className="flex justify-between items-end">
                          <div className="space-y-1">
                            <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest block">Melhor Preço</span>
                            <span className="text-xs font-bold text-slate-900 uppercase tracking-tight truncate max-w-[140px] block">{product.bestMerchant}</span>
                          </div>
                        ...
                        <div className="flex gap-3 mt-4">
                          <button
                            ...
                            className="flex-1 py-4 bg-slate-900 text-white rounded-xl font-bold text-[10px] uppercase tracking-widest hover:bg-slate-800 transition-all flex items-center justify-center gap-2.5 shadow-lg shadow-black/5"
                          >
                  ...
                  {shoppingList.length === 0 ? (
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                      {Array.from(new Set(shoppingList.map(i => i.bestMerchant))).map(merchant => (
                        <div key={merchant} className="bg-white/5 border border-white/5 rounded-[32px] p-10 space-y-8 hover:bg-white/[0.07] transition-all">
                          <div className="flex items-center gap-4 border-b border-white/10 pb-6">
                            <div className="w-10 h-10 bg-brand-400/20 text-brand-400 rounded-xl flex items-center justify-center">
                              <Store size={20} />
                            </div>
                            <h3 className="font-bold uppercase text-[10px] tracking-[0.2em] truncate text-slate-200">{merchant}</h3>
                          </div>
                          <div className="space-y-6">
                            {shoppingList.filter((i: any) => i.bestMerchant === merchant).map((item: any) => (
                              <div key={item.id} className="flex justify-between items-center group">
                                <span className="text-slate-400 truncate max-w-[180px] font-medium text-sm transition-colors group-hover:text-white">{item.name}</span>
                                <span className="font-bold text-white text-base">{formatCurrency(item.minPrice)}</span>
                              </div>
                            ))}
                          </div>
                        </div>
          ...
          <div className="space-y-12 animate-in fade-in duration-700">
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
              <div className="lg:col-span-2 bg-slate-900 rounded-[48px] p-12 sm:p-16 text-white relative overflow-hidden border border-slate-800 shadow-2xl">
                <div className="relative z-10 space-y-12">
                  <div className="flex items-center gap-6">
                    <div className="p-4 bg-brand-500/10 rounded-2xl border border-white/5">
                      <Calendar size={28} className="text-brand-400" />
                    </div>
                    <div className="space-y-1">
                      <h3 className="text-[10px] font-bold uppercase tracking-[0.3em] text-slate-500">Análise de Consumo</h3>
                      <h2 className="text-4xl font-bold tracking-tight leading-tight">Laboratório de <br />Inflação</h2>
                    </div>
                  </div>
                  <p className="text-slate-400 max-w-md font-medium text-lg leading-relaxed">Acompanhe como os preços dos produtos que você realmente consome variam ao longo do tempo.</p>
                  <div className="pt-10 border-t border-white/5 flex gap-12">
                    <div>
                      <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1.5">Produtos Monitorados</p>
                      <p className="text-4xl font-bold tracking-tight">{comparisonData.length}</p>
                    </div>
                    <div>
                      <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1.5">Segmentação</p>
                      <p className="text-4xl font-bold tracking-tight text-brand-400">Smart</p>
                    </div>
                  </div>
                </div>
              </div>

              <div className="bg-white rounded-[40px] border border-slate-100 p-12 shadow-soft flex flex-col justify-center items-center text-center space-y-8">
                <div className="w-20 h-20 bg-slate-50 rounded-3xl flex items-center justify-center text-slate-900">
                  <BarChart3 size={40} />
                </div>
                <div className="space-y-3">
                  <h3 className="text-2xl font-bold text-slate-900 tracking-tight">Estatísticas</h3>
                  <p className="text-slate-500 font-medium leading-relaxed">Dados processados em tempo real com hardware de aceleração IA.</p>
                </div>
                <button
                  onClick={fetchIntelligenceData}
                  className="px-8 py-3 bg-slate-100 text-slate-950 rounded-xl font-bold text-[10px] uppercase tracking-widest hover:bg-slate-200 transition-all border border-slate-200/50"
                >
                  Atualizar Dados
                </button>
              </div>
            ...
            <div className="bg-white rounded-[40px] border border-slate-100 shadow-soft overflow-hidden">
              <div className="p-10 border-b border-slate-50 flex items-center justify-between">
                <h3 className="font-bold text-slate-900 uppercase tracking-[0.2em] text-[10px]">Variabilidade por Item</h3>
                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest bg-slate-50 px-3 py-1.5 rounded-lg border border-slate-100">Top 10 Itens</span>
              </div>
              <div className="divide-y divide-slate-50">
                {comparisonData.slice(0, 10).map((product, idx) => (
                  <div key={idx} className="p-10 flex items-center justify-between hover:bg-slate-50/50 transition-all group cursor-pointer" onClick={() => setSelectedProduct(product)}>
                    <div className="flex items-center gap-8">
                      <div className="text-4xl font-bold text-slate-100 group-hover:text-slate-200 transition-colors tracking-tighter">{(idx + 1).toString().padStart(2, '0')}</div>
                      <div className="space-y-1">
                        <h4 className="font-bold text-slate-900 uppercase text-base tracking-tight leading-none group-hover:text-brand-600 transition-colors">{product.name}</h4>
                        <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">{product.category}</p>
                      </div>
                    </div>
                    <div className="text-right space-y-1">
                      <p className={`text-xl font-bold tracking-tight ${product.trend === 'up' ? 'text-rose-600' : 'text-emerald-600'}`}>
                        {product.trend === 'up' ? '+' : '-'}{Math.round(Math.abs((product.lastPrice - product.avgPrice) / product.avgPrice) * 100)}%
                      </p>
                      <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest block">vs Média</span>
                    </div>
                  </div>
                ))}
              </div>
      ...
      {selectedProduct && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
          <div className="absolute inset-0 bg-slate-900/40 backdrop-blur-md transition-opacity" onClick={() => setSelectedProduct(null)}></div>
          <div className="relative bg-white w-full max-w-2xl rounded-[48px] shadow-soft-lg overflow-hidden flex flex-col max-h-[90vh] animate-in zoom-in-95 duration-300">
            <div className="p-10 sm:p-14 border-b border-slate-50 flex justify-between items-start gap-8">
              <div className="space-y-3">
                <span className="text-[10px] font-bold text-brand-600 uppercase tracking-[0.2em] px-3 py-1.5 bg-brand-50 rounded-lg border border-brand-100 w-fit block">Analítico AI</span>
                <h2 className="text-3xl font-bold text-slate-900 tracking-tight uppercase leading-tight">{selectedProduct.name}</h2>
              </div>
              <button
                onClick={() => setSelectedProduct(null)}
                className="w-12 h-12 bg-slate-50 text-slate-400 hover:text-slate-900 hover:bg-slate-100 rounded-2xl flex items-center justify-center transition-all shrink-0"
              >
                <X size={24} />
              </button>
            </div>

            <div className="flex-1 overflow-y-auto p-10 sm:p-14 space-y-12 scrollbar-hide">
              <div className="grid grid-cols-2 gap-6">
                <div className="bg-emerald-50/30 p-8 rounded-[32px] border border-emerald-100/50 space-y-2">
                  <p className="text-[10px] font-bold text-emerald-600 uppercase tracking-widest">Mínimo Histórico</p>
                  <p className="text-3xl font-bold text-emerald-600 tracking-tighter">{formatCurrency(selectedProduct.minPrice)}</p>
                </div>
                <div className="bg-slate-50/50 p-8 rounded-[32px] border border-slate-100 space-y-2">
                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Preço Médio</p>
                  <p className="text-3xl font-bold text-slate-900 tracking-tighter">{formatCurrency(selectedProduct.avgPrice)}</p>
                </div>
              </div>

              <div className="space-y-6">
                <div className="flex items-center justify-between border-b border-slate-50 pb-4">
                  <h4 className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em]">Histórico de Lançamentos</h4>
                  <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{selectedProduct.history.length} Registros</span>
                </div>
                <div className="space-y-4">
                  {selectedProduct.history.map((price: any, i: number) => (
                    <div key={i} className="flex justify-between items-center p-6 bg-white border border-slate-100 rounded-2xl hover:border-slate-200 transition-all hover:shadow-soft group">
                      <div className="flex items-center gap-5">
                        <div className="w-12 h-12 bg-slate-50 rounded-xl flex items-center justify-center text-slate-300 group-hover:bg-slate-900 group-hover:text-white transition-all">
                          <Store size={20} />
                        </div>
                        <div className="space-y-1">
                          <p className="font-bold text-slate-900 text-sm uppercase truncate max-w-[200px] tracking-tight">{price.ai_documents?.merchant_raw || 'Ponto de Venda'}</p>
                      ...
                      <div className="text-right space-y-1.5">
                        <p className="font-bold text-slate-900 text-lg tracking-tight">{formatCurrency(price.unit_price)}</p>
                        {price.is_promo && (
                          <span className="text-[8px] font-bold text-rose-500 uppercase tracking-widest bg-rose-50 px-2 py-1 rounded-md border border-rose-100">Promoção</span>
                        )}
                      </div>
                    </div>
                  ))}
            ...
            <div className="p-10 sm:p-14 bg-slate-50 border-t border-slate-100">
              <button
                onClick={() => {
                ...
                className="w-full h-20 bg-slate-900 text-white rounded-[28px] font-bold text-xs uppercase tracking-[0.2em] hover:bg-slate-800 transition-all flex items-center justify-center gap-3 active:scale-95 shadow-xl shadow-black/5"
              >
