...
// Mesma regra de fechamento/vencimento de lib/historyUtils.ts (getCardCompetenceDate) —
// duplicada aqui de propósito em vez de importada: os outros arquivos de api/_lib/ nunca
// importam de lib/ (só de outros arquivos dentro do próprio api/_lib/), e esse é o
// caminho já comprovado como seguro no empacotamento das functions da Vercel.
const getCardCompetenceDate = (purchaseDateISO: string, closingDay: number, dueDay: number): string => {
...
// Mesma regra de lib/dateUtils.ts (addDaysISO), duplicada pelo mesmo motivo acima.
const addDaysISO = (dateStr: string, delta: number): string => {
...
async function loadCardCompetenceHelpers(supabase: any, userId: string) {
  ...
  const getCompetenceDate = (ct: any): string => {
    ...
    return getCardCompetenceDate(ct.date, meta.closingDay, meta.dueDay);
  };
...
async function toolGetTransactions(supabase: any, userId: string, args: any) {
  ...
  const [txRes, cardRes] = await Promise.all([
    ...
    supabase.from('card_transactions')
      ...
      .eq('user_id', userId)
      .gte('date', addDaysISO(startDate, -92))
      .lte('date', endDate),
  ]);
