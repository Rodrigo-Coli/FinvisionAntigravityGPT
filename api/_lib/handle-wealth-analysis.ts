...
export async function handleWealthAnalysis(req: any, res: any) {
    ...
    try {
        ...
        const liabilitiesSummary = liabilities.map((l: any) => { const ir = l.interest_rate ? `juros ${l.interest_rate}% a.a.` : 'sem juros definidos'; const parcelas = l.installments_remaining ? `${l.installments_remaining} parcelas restantes de R$ ${Number(l.installment_amount).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}` : 'sem parcelas configuradas'; return `${l.name} (${l.type}): Saldo Devedor R$ ${Number(l.remaining_balance).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}, ${ir}, ${parcelas}`; }).join('\n- ');
