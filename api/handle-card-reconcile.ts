...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    const prompt = `Você é um especialista em conciliação de cartões de crédito. Analise o documento fornecido. OBJETIVO: Extrair transações e identificar colunas de forma inteligente. REGRAS: 1. DATA (YYYY-MM-DD). 2. DESCRIÇÃO. 3. VALOR. 4. CARTÃO (map p/ card_name). 5. CATEGORIA. PARCELAMENTO: Se a descrição contiver '2/5', extraia installment_number=2 e installment_total=5. Retorne APENAS JSON: {"transactions": [{"date": "YYYY-MM-DD", "description": "texto", "amount": 123.45, "installment_number": 2, "installment_total": 5, "card_name": "Nome do Cartão", "category_name": "Nome da Categoria"}]}`;
        ...
        for (const t of processedTxs) {
          ...
          if (!cardId) {
            ...
            if (exCard) cardId = exCard.id;
              ...
              const { data: newCard } = await supabase.from('cards').insert({ user_id: imp.user_id, name: t.card_name || 'Mobills Card', brand: 'VISA', limit_total: 1000, closing_day: 1, due_day: 10, color: '#6366f1', account_id: targetAccountId }).select('id').single();
              cardId = newCard?.id;
            }
