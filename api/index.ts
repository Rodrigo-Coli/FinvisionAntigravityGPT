export default async function handler(req: any, res: any) {
  ...
  try {
    // Switch to dynamic indexing for resilience - pointing to correct src path
    switch (funcName) {
      ...
      default:
        return res.status(404).json({ error: 'END_POINT_NOT_MAPPED', funcName });
    }
  } catch (error: any) {
    console.error(`[Router] Error loading API endpoint ${funcName}:`, error);
