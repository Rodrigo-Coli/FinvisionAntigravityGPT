...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    if (!geminiKey) {
      throw new Error('Chave do Gemini (GEMINI_API_KEY) não encontrada.');
    }

    const ai = new GoogleGenAI({ apiKey: geminiKey });
    const model = 'gemini-2.5-flash';

    console.log(`[Categorize] Analyzing ${descriptions.length} unique descriptions with ${categories.length} categories.`);
...
${JSON.stringify(categories.map(c => c.category_name + " > " + c.name), null, 2)}
    ...
    const schema: Schema = {
      ...
      items: {
        ...
        properties: {
          description: {
            type: Type.STRING,
            description: "The exact raw description string from the input array."
          },
          category: {
            type: Type.STRING,
            description: "The matching category name from the available options (e.g. Transporte). Leave empty if unknown."
          },
          subcategory: {
             type: Type.STRING,
             description: "The matching subcategory name from the available options (e.g. Combustível). Leave empty if unknown."
          }
        },
