...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    let contents: any;
    if (isTextFile) contents = { parts: [{ text: prompt }, { text: `CONTEÚDO DO ARQUIVO:\n${buffer.toString('utf-8').substring(0, 30000)}` }] };
    else contents = { parts: [{ text: prompt }, { inlineData: { data: buffer.toString('base64'), mimeType: doc.mime_type || 'application/pdf' } }] };
