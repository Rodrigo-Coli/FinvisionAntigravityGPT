...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    const prompt = `
      ...
      4. PARCELAMENTO: Se a descrição contiver algo como 'Amazon 2/5' ou '2 de 5', extraia installment_number=2 e installment_total=5.
      5. REMOVA: Linhas de SALDO, TOTAL, LIMITE, PAGAMENTO MINIMO, IOF, JUROS, ou qualquer linha que não seja uma transação financeira.
    ...
    const txsToInsert = processedTxs.map((t: any) => {
      ...
      // Default date to today in Sao Paulo timezone if IA fails to extract
