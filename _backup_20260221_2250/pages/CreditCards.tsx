...
const CreditCardsPage: React.FC = () => {
  ...
  const fetchCards = async () => {
    ...
    try {
      ...
      if (window.location.href.includes('dev=true')) {
        const mockCards = [
          { id: 'c1', name: 'Mastercard Black', brand: 'Mastercard', last4: '8899', limit_total: 50000, closing_day: 5, due_day: 15, color: 'bg-slate-900' },
          { id: 'c2', name: 'Visa Infinite', brand: 'Visa', last4: '1122', limit_total: 25000, closing_day: 10, due_day: 20, color: 'bg-brand-600' }
        ];
        setCards(mockCards);
        if (!selectedCard) setSelectedCard(mockCards[0]);
        setLoading(false);
        return;
      }
  ...
  const loadCardContext = async (cardId: string) => {
    try {
      ...
      console.log(`[Card-Logic] Buscando fatura alvo: ${targetMonth}/${targetYear} para o cartão ${card?.name}`);
  ...
  const fetchTransactions = async (cardId: string, statementId?: string | null) => {
    ...
    try {
      ...
      if (window.location.href.includes('dev=true')) {
        const mockTxs = [
          { id: 't1', date: new Date().toISOString(), description: 'Apple Services', amount: 49.90, category_name: 'Tech' },
          { id: 't2', date: new Date().toISOString(), description: 'Restaurante Zen', amount: 150.00, category_name: 'Lazer' },
          { id: 't3', date: new Date().toISOString(), description: 'Posto Shell', amount: 300.00, category_name: 'Transporte' }
        ];
