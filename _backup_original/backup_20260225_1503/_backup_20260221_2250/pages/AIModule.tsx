...
const AIModule: React.FC<{ user: Profile }> = ({ user }) => {
  ...
  const fetchAccounts = async () => {
    ...
    if (window.location.href.includes('dev=true')) {
      const mockAccounts = [{ id: 'a1', institution: 'Nubank' }, { id: 'a2', institution: 'Bradesco' }];
      setSelectedAccounts(mockAccounts);
      setTargetAccount(mockAccounts[0].id);
      return;
    }
  ...
  const fetchIntelligenceData = async () => {
    ...
    try {
      ...
      if (window.location.href.includes('dev=true')) {
        const mockData = [
          { id: '1', name: 'Leite Integral 1L', category: 'Laticínios', merchantCategory: 'Mercado', avgPrice: 5.50, minPrice: 4.89, bestMerchant: 'Pão de Açúcar', lastPrice: 5.20, trend: 'down' },
          { id: '2', name: 'Café Torrado 500g', category: 'Grãos', merchantCategory: 'Mercado', avgPrice: 18.20, minPrice: 15.90, bestMerchant: 'Carrefour', lastPrice: 19.50, trend: 'up' },
          { id: '3', name: 'Sabonete Dove 90g', category: 'Higiene', merchantCategory: 'Mercado', avgPrice: 4.50, minPrice: 3.99, bestMerchant: 'Drogasil', lastPrice: 4.40, trend: 'down' }
        ];
