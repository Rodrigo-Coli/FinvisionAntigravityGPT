...
try {
    ...
    $files = @(
        "types.ts", "App.tsx", "index.tsx", "index.html", "package.json", "vite.config.ts", "vercel.json",
        "lib/supabase/client.ts", "lib/dateUtils.ts", "lib/historyUtils.ts", "lib/transactionSeriesUtils.ts",
        ...
        "api/handle-receipt-items.ts", "api/handle-bank-reconcile.ts", "api/handle-card-reconcile.ts", "supabase/master_migration.sql", "supabase/clear_data.sql"
    )
    ...
    Write-Host "Commit..."
    $commitBody = @{
        message = "Atomic Deploy: Phase 3 FinVision and Phase 4 Tour System"
