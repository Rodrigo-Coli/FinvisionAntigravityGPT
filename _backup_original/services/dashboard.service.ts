...
export const DashboardService = {
  getSummary: async (): Promise<DashboardData> => {
    ...
    const activeAccounts = (accounts || []).filter((acc: any) => acc.is_archived === false || acc.status === 'active' || !acc.is_archived);
    ...
    const activeCards = (cards || []).filter((c: any) => c.is_archived === false || c.status === 'active' || !c.is_archived);

    // We'll need the current statements for these cards
    const creditCardsSummary = await Promise.all(activeCards.map(async (card: any) => {
